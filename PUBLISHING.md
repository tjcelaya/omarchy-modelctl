...
inside the plugin folder. The plugin id must be globally unique across the marketplace
...
3. Tag the release: `git tag v0.1.1 && git push --tags` (bump `version` in
...
5. Submit: https://github.com/omacom/omarchy-plugin-marketplace/issues/new?template=submit-plugin.yml
   ...
   - Maintainer notes: ships systemd *user* units and an `install.sh` that only
     symlinks into `~/.local/bin` and `~/.config/systemd/user`; no sudo/pkexec, no
     downloads, nothing runs at boot. Expect the automated baseline to report the
